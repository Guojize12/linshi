...
void APP_USER_inspectionStop(uint8_t index)
{
    ...
    {   
        ...
        APP_DWIN_SUB_Event_clean();  //ѲвĲ״̬
...
void APP_COMM_4009_SetInspectionAllowUpdate(uint8_t index)
{
    ...
    if(elevator_info[index].inspection_enable)
    {
        ...
        if(object_info.inspection_status) //Ѳ
        {
            if(elevator_info[index].errStop == 0  //Ƿ쳣ͣ
                && elevator_info[index].FxDoorSta[1].bt8_2.bit0 == 0  //״̬
                && elevator_info[index].ctlMode == CTL_MODE_AUTO)  //Զģʽ
            {
